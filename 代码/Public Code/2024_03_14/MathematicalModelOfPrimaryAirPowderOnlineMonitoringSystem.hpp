...
private:
	...
	*/
		...
		bool unit_temperature = UNITOPTION::TEMPERATUREINCELSIUS)
	{
		...
		*specific_heat_of_air = -0.415813 * pow(temperature, 4)
			+ 1.70338 * pow(temperature, 3)
			- 1.90478 * pow(temperature, 2)
			+ 0.890486 * pow(temperature, 1)
			+ 0.86751;
		*specific_heat_of_pulverized_coal = 0.00865 * pow(temperature, 4)
			- 0.16581 * pow(temperature, 3)
			+ 0.3307 * pow(temperature, 2)
			+ 0.01550 * pow(temperature, 1)
			+ 1.151;
	}
public:
	...
	*/
		...
		double temperature_3)
	{
		...
		*pulverized_coal_concentration =
			(specific_heat_of_air_1 * temperature_1 - specific_heat_of_air_2 * temperature_3)
			/ (specific_heat_of_pulverized_coal_2 * temperature_3 - specific_heat_of_pulverized_coal_1 * temperature_2)
			- 0.005;
	}
	...
	*/
	static void pulverizedCoalMassFlowRate(double* pulverized_coal_mass_flow_rate,
		...
		double the_density_of_air_under_standard_conditions,
...

	{
		...
		*pulverized_coal_mass_flow_rate = 3600 * wind_speed
			* cross_sectional_area_of_primary_duct
			* the_density_of_air_under_standard_conditions
			* (273 / primary_air_temperature)
			* pulverized_coal_concentration;
	}
